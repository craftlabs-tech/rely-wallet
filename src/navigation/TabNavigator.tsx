...
import { Animated, BackHandler, Platform, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
export function TabNavigator() {
  ...
  return (
    <Tab.Navigator
      ...
      <Tab.Screen name="activity" component={Example} options={{ tabBarLabel: 'Activity' }} />
    </Tab.Navigator>
  );
}
...
function TabBar({ state, descriptors, navigation, position }: MaterialTopTabBarProps) {
  ...
  const { borders, colors, components, gutters, fonts, layout } = useTheme();
  ...
  return (
    <View
      style={[
        ...
        {
          borderWidth: insets.bottom === 0 ? 0 : 1,
        ...
        {state.routes.map((route, index) => {
          ...
          const renderIcon = (name: string) => {
            switch (name) {
              ...
              case 'home':
                return <Ionicons name="wallet" size={26} color={colors.text_01} />;
              default:
                return null;
            }
          };
