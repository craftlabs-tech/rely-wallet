...
import type { EIP712 } from '@/utils/wallet';

import { WalletKit } from '@reown/walletkit';
import { Core } from '@walletconnect/core';
import { buildApprovedNamespaces, getSdkError } from '@walletconnect/utils';
import { toUtf8String, Wallet } from 'ethers';
 ...
 */
let syncWalletClient: Awaited<ReturnType<(typeof WalletKit)['init']>> | undefined;

const core = new Core({ projectId: process.env.WALLETCONNECT_PROJECT_ID });

const walletClient = WalletKit.init({ core, metadata });
...
function onSessionProposal({ id, params, verifyContext }: WalletKitTypes.SessionProposal) {
  ...
  console.log(supportedNamespaces.solana);
  ...
  console.log('onSessionProposal', props);

  navigationRef.navigate(Paths.SessionProposal, props);
}

function onProposalExpire({ id }: WalletKitTypes.ProposalExpire) {
...
function onSessionDelete({ id, topic }: WalletKitTypes.SessionDelete) {
  logger.debug(`session_delete: ${JSON.stringify({ id, topic })}`);
}

function onSessionAuthenticate({ id, topic, params }: WalletKitTypes.SessionAuthenticate) {
  logger.debug(`session_authenticate: ${JSON.stringify({ id, topic, params })}`);
}

async function onSessionRequest(sessionRequest: WalletKitTypes.SessionRequest) {
  ...
  if (params.request.method === 'personal_sign') {
    ...
    const onApprove = async () => {
      try {
        ...
        const evmWallet = await createEVMWallet(mnemonic as string, 0);
        const wallet = new Wallet(evmWallet.privateKey);
    ...
    navigationRef.navigate(Paths.SessionRequest, props);
    return;
  }
  ...
  if (params.request.method === 'eth_signTypedData_v4') {
    ...
    navigationRef.navigate(Paths.SessionRequest, {
...
function onSessionRequestExpire({ id }: WalletKitTypes.SessionRequestExpire) {
...
export function buildNamespaces({ params }: WalletKitTypes.SessionProposal) {
  ...
  console.log('buildNamespaces', params.requiredNamespaces.solana);
  ...
  const namespaces: Namespace = {
    ...
    solana: {
      accounts: Array.from(
        new Set(
          (
            params.requiredNamespaces.solana?.chains?.map((chainId) => `${chainId}:${solanaWallet?.address}`) ?? []
          ).concat(
            params.optionalNamespaces.solana?.chains?.map((chainId) => `${chainId}:${solanaWallet?.address}`) ?? [],
          ),
        ),
      ),
...
export function isChainSupported(chainId: string) {
  return chainId.startsWith('eip155:') || chainId.startsWith('solana:') || chainId.startsWith('mina:');
}
