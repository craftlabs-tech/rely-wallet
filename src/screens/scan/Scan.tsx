...
function Scan() {
  ...
  const paste = async () => {
    ...
    if (uri && isWalletConnectURI(uri) && isWalletConnectURIV2(uri)) {
      void pair(uri);
    }
  };
  ...
  if (hasPermission) {
    return (
      <View style={[layout.flex_1, layout.justifyAround, layout.itemsCenter, backgrounds.ui_background]}>
  ...
  return (
    <View style={[layout.flex_1]}>
      <View style={[layout.absolute, layout.top0, layout.row, layout.center, layout.fullWidth, gutters.padding_10]}>
        <View style={components.indicator} />
      </View>
      <View style={[layout.flex_1, layout.justifyAround, layout.itemsCenter]}>
        ...
        <View style={[layout.row, layout.center, { marginTop: width * 0.8 + height * 0.12 }]}>
          ...
          <TouchableOpacity style={gutters.paddingLeft_6} onPress={paste}>
            <Ionicons name="paste" color={colors.text_02} size={24} />
          </TouchableOpacity>
        </View>
        <TouchableOpacity
          onPress={() => navigation.navigate(Paths.WalletconnectSessions)}
